...
export const Shop: React.FC<ShopProps> = ({ isOpen, onClose, money, onBuy, currentSeason, inventory, onSell, unlockedAreas, t }) => {
      ...
      <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[85vh] relative border-4 border-rose-100">
        ...
        <div className="p-6 pb-4 flex justify-between items-center bg-rose-50 z-10 border-b border-rose-100">
          <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-2xl font-black text-slate-800">{t('GENERAL_STORE')}</h2>
                <p className="text-xs text-slate-500 font-bold">{mode === 'BUY' ? t('BUY_SEEDS') : t('SELL_PRODUCE')}</p>
            </div>
          </div>
          
          <div className="flex items-center gap-4">
              <div className="bg-white/80 px-4 py-2 rounded-full border border-rose-100 flex items-center gap-2">
                  <span className="text-xs font-bold text-slate-400 uppercase">{t('FUNDS')}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="flex gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-slate-400 font-medium leading-tight mt-1 h-8 line-clamp-2">
                                {isUnlocked ? crop.description : `${t('UNLOCK_VIA')} ${requiredAreaName}`}
                            </p>
                        </div>
                    </div>
                    <div className="mt-4 flex items-center justify-between pt-3 border-t border-slate-50">
                        <div className="flex flex-col">
                            <span className="text-[10px] font-bold text-slate-400 uppercase">{t('PRICE')}</span>
                        ...
                        <button 
                            ...
                            `}
                        >
                            {isUnlocked ? <><ShoppingBag className="w-4 h-4" /> {t('BUY')}</> : <Lock className="w-3 h-3" />}
                        </button>
                    </div>
                </div>
              );
            ...
            {mode === 'SELL' && CROPS.map((crop) => {
               ...
               return (
                <div key={crop.id} className={`flex flex-col p-4 rounded-2xl bg-white shadow-sm hover:shadow-md transition border group ${isProduct ? 'border-blue-100' : 'border-emerald-100'}`}>
                    <div className="flex gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                <span className="text-xs font-bold text-slate-400 uppercase">{t('OWNED')}:</span>
                    ...
                    <div className="mt-4 flex items-center justify-between pt-3 border-t border-slate-50">
                        <div className="flex flex-col">
                            <span className="text-[10px] font-bold text-slate-400 uppercase">{t('VALUE')}</span>
            ...
            {mode === 'SELL' && Object.keys(inventory).every(key => {
                ...
                <div className="col-span-full flex flex-col items-center justify-center py-12 text-slate-400">
                    ...
                    <p className="font-bold">{t('NO_SELL')}</p>
                </div>
            )}
